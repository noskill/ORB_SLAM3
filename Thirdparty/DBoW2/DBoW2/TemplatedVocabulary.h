...
void TemplatedVocabulary<TDescriptor,F>::load_json(const std::string &filename) {
     ...
     assert(document.HasMember("vocabulary"));
     const Value & fvoc = document["vocabulary"];
     assert(fvoc.IsObject());

     m_k = fvoc["k"].GetInt();
     m_L = fvoc["L"].GetInt();
     m_scoring = (ScoringType)(fvoc["scoringType"].GetInt());
     m_weighting = (WeightingType)(fvoc["weightingType"].GetInt());

     createScoringObject();

     // nodes
     const Value & nodes = fvoc["nodes"];

     m_nodes.resize(nodes.Size() + 1); // +1 to include root
     m_nodes[0].id = 0;
     ...
     {
       NodeId wid = words[i]["wordId"].GetInt();
       NodeId nid = words[i]["nodeId"].GetInt();
