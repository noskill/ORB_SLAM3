...
public:
  ...
  QueryResults query(const std::vector<TDescriptor> &features,
...
{
	return add(features, nullptr, nullptr);
}
...
QueryResults TemplatedDatabase<TDescriptor, F>::query(
  const std::vector<TDescriptor> &features, int max_results, int max_id) const
{
   QueryResults result;
   this->query(features, result, max_results, max_id);
   return result;
}
