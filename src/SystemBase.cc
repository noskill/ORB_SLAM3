...
SystemBase::SystemBase(const eSensor sensor, const bool bUseViewer):
    mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false),
	mbResetActiveMap(false), mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false) {}


cv::Mat SystemBase::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
...
cv::Mat SystemBase::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, string filename)
{
    if(mSensor!=RGBD)
    {
...
cv::Mat SystemBase::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
...
void SystemBase::ActivateLocalizationMode()
{
    unique_lock<mutex> lock(mMutexMode);
    mbActivateLocalizationMode = true;
}

void SystemBase::DeactivateLocalizationMode()
{
    unique_lock<mutex> lock(mMutexMode);
    mbDeactivateLocalizationMode = true;
}

bool SystemBase::MapChanged()
{
    static int n=0;
...
void SystemBase::ResetActiveMap()
{
    unique_lock<mutex> lock(mMutexReset);
...
void SystemBase::SaveTrajectoryTUM(const string &filename)
{
...
void SystemBase::SaveKeyFrameTrajectoryTUM(const string &filename)
{
...
void SystemBase::SaveTrajectoryEuRoC(const string &filename)
{
...
void SystemBase::SaveKeyFrameTrajectoryEuRoC(const string &filename)
{
    cout << endl << "Saving keyframe trajectory to " << filename << " ..." << endl;
...
void SystemBase::SaveTrajectoryKITTI(const string &filename)
{
    cout << endl << "Saving camera trajectory to " << filename << " ..." << endl;
    if(mSensor==MONOCULAR)
    {
...
void SystemBase::SaveDebugData(const int &initIdx)
{
...
int SystemBase::GetTrackingState()
{
    unique_lock<mutex> lock(mMutexState);
    return mTrackingState;
}

vector<MapPoint*> SystemBase::GetTrackedMapPoints()
{
    unique_lock<mutex> lock(mMutexState);
    return mTrackedMapPoints;
}

vector<cv::KeyPoint> SystemBase::GetTrackedKeyPointsUn()
{
    unique_lock<mutex> lock(mMutexState);
    return mTrackedKeyPointsUn;
}

double SystemBase::GetTimeFromIMUInit()
{
...
bool SystemBase::isFinished()
{
    return (GetTimeFromIMUInit()>0.1);
}

void SystemBase::ChangeDataset()
{
...
/*void SystemBase::SaveAtlas(int type){
...
bool SystemBase::LoadAtlas(string filename, int type)
{
...
string SystemBase::CalculateCheckSum(string filename, int type)
{
    string checksum = "";
