...
std::function<float(cv::Mat, cv::Mat)> ORBmatcher::DISTANCE = ORBmatcher::DescriptorDistance;

const int ORBmatcher::HISTO_LENGTH = 30;

void ORBmatcher::set_th_low(float value){
    ORBmatcher::TH_LOW = value;
}

void ORBmatcher::set_th_high(float value){
    ORBmatcher::TH_HIGH = value;
}
        ...
        if(pMP->mbTrackInView)
        {
            ...
            if(!vIndices.empty()){
                ...
                for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
                {
                    ...
                    const float dist = getDistance()(MPdescriptor,d);
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                for(size_t iF=0; iF<vIndicesF.size(); iF++)
                {
                    if(F.Nleft == -1){
                        ...
                        const float dist =  getDistance()(dKF,dF);
                                   ...
                                   vector<MapPoint*> &vpMatched, int th, float ratioHamming)
{
    ...
    for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
    {
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            const float dist = getDistance()(dMP,dKF);
        ...
        if(f1it->first == f2it->first)
        {
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    const float dist = getDistance()(d1,d2);
...
int ORBmatcher::Fuse(KeyFrame *pKF, cv::Mat Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
{
    ...
    for(int iMP=0; iMP<nPoints; iMP++)
    {
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(); vit!=vIndices.end(); vit++)
        {
            ...
            float dist = getDistance()(dMP,dKF);
            ...
            if(pMP)
            {
                if(!LastFrame.mvbOutlier[i])
                {
                    ...
                    for(vector<size_t>::const_iterator vit=vIndices2.begin(), vend=vIndices2.end(); vit!=vend; vit++)
                    {
                        ...
                        const float dist = getDistance()(dMP,d);
...
std::function<float(cv::Mat, cv::Mat)> ORBmatcher::getDistance(){
    return DISTANCE;
}

std::function<float(cv::Mat, cv::Mat)> ORBmatcher::setDistance(std::function<float(cv::Mat, cv::Mat)> dist){
